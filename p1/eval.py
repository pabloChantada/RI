...
def run_evaluation(env, model, max_steps=30, deterministic=True):
    """
    ...
    Args:
        ...
        deterministic: Si True, usa acciones deterministas
    ...
    a0, t0 = pos(info)
    traj_agent.append(a0)
    traj_target.append(t0)
...
def plot_trajectories(traj_agent, traj_target, episode_info, half=1000, out_name="trajectories_2d.png"):
    ...
    # Trayectoria del agente con gradiente de color
    agent_x = [p[0] for p in traj_agent]
    agent_z = [p[1] for p in traj_agent]
    
    # Plot con puntos coloreados por tiempo
    scatter = ax.scatter(agent_x, agent_z, c=range(len(agent_x)), 
                        cmap='viridis', s=50, alpha=0.6, 
                        label='Trayectoria agente')
    ax.plot(agent_x, agent_z, 'b-', alpha=0.3, linewidth=1)
    ...
    ax.scatter(agent_x[0], agent_z[0], marker='o', s=200, 
              color='green', edgecolors='black', linewidths=2,
              label='Inicio agente', zorder=5)
    ax.scatter(target_x[0], target_z[0], marker='x', s=200,
    ...
    # Círculo de objetivo (radio 150)
    for tx, tz in zip(target_x[::5], target_z[::5]):  # Cada 5 posiciones
        circle = plt.Circle((tx, tz), 150, color='red', 
                           fill=False, linestyle=':', alpha=0.2)
        ax.add_patch(circle)
    
    # Colorbar para el tiempo
    cbar = plt.colorbar(scatter, ax=ax)
    cbar.set_label('Paso de tiempo', rotation=270, labelpad=20)
    ...
    title += f"Pasos: {episode_info['total_steps']} | "
...
def run_multiple_evaluations(env, model, num_evals=5, deterministic=True):
    """
    ...
    for i in range(num_evals):
        ...
        traj_agent, traj_target, reward, info = run_evaluation(
            env, model, deterministic=deterministic
        )
...
if __name__ == "__main__":
    ...
    # Cargar modelo
    model_path = os.path.join(MODELS_DIR, "sac_cylinder.zip")
    ...
    run_multiple_evaluations(env, model, num_evals=5, deterministic=True)
