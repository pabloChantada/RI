...
class CylinderEnv(gym.Env):
    def __init__(self, size: int = 10):
        ...
        _object_id = list(self.sim.getObjects())[0] # Theres only one object in the scene
        self._target_location_x, self._target_location_z = self._get_object_position()
        self._init_rob_position_x, self._init_rob_position_z = self._get_agent_position()
        ...
        def _get_object_position(self):
            _object_id = list(self.sim.getObjects())[0]  # Theres only one object in the scene
            target_location = self.sim.getObjectLocation(_object_id)
        ...
        def _get_obs(self):
            ...
            target_position = self._get_object_position()
